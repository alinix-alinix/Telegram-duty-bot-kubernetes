...
TOKEN = os.getenv("GLCI_TELEGRAM_BOTID")
CHAT_ID = os.getenv("GLCI_TELEGRAM_CHATID")

# Список юзернеймов, которые могут стать дежурными
allowed_users = os.getenv("GLCI_ALLOWED_USERS").split(":")
...
async def call_duty(message: Message):
    print("Команда /call вызвана")  # Отладочный вывод в консоль
    if current_duty:
...
async def handle_channel_post(message: Message):
    ...
    if mentioned:
        ...
        if message.chat.username:
            ...
            chat_title = message.chat.title if message.chat.title else "Неизвестный канал"
            message_link = f"Ссылка на сообщение недоступна: НЕПУБЛИЧНЫЙ КАНАЛ\n Имя канала: {chat_title}.\nТекст сообщения: {message.text}"

        # Если дежурный не назначен — отправляем в общий чат инженеров
        if current_duty is None:
            await bot.send_message(
                CHAT_ID,
                f"⚠️ Внимание! Дежурный не назначен.\nСообщение из канала: {message_link}"
            )
            return

        # Если дежурный есть, отправляем сообщение ему
        await bot.send_message(current_duty, f"Сообщение из канала:\n{message_link}")
